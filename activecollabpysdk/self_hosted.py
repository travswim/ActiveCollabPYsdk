...
class SelfHosted(Authenticator):
    ...
    def __init__(
        ...
        if not all([result.scheme, result.netloc]):
            raise InvalidArgumentError(
                f'Self hoste URL {self_hosted_url} is not valid'
                )
        ...
        if api_version <= 0:
            raise InvalidArgumentError(
                f'{api_version} is an invalid api version'
                )
    ...
    def issueToken(self):
        ...
        request_url = f'{self.self_hosted_url}/api/v\
            {self.api_versionself_hosted_url}/issue-token'
        ...
        if r.json() and r.headers['content-type'] == 'application/json':
            ...
            if not response['is_ok'] or not response['token']:
                ...
                raise AuthenticationError(
                    f'Invalid response from {request_url}. JSON expected, got \
                        {contentType}, status code {r.status_code}')
